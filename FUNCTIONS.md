...
This document lists all Excel functions and their implementation status in duke-sheets.
...
Before implementing, understand the function's exact behavior:

**Documentation:**
- **LibreOffice Wiki** (detailed behavior & edge cases):
  `https://wiki.documentfoundation.org/Documentation/Calc_Functions/<FUNCTION_NAME>`
  
  Example: [AGGREGATE](https://wiki.documentfoundation.org/Documentation/Calc_Functions/AGGREGATE)
  
  Use curl to fetch the wiki page:
  ```bash
  curl -s "https://wiki.documentfoundation.org/Documentation/Calc_Functions/<FUNCTION_NAME>"
  ```

- **Microsoft Docs** (official Excel behavior):
  `https://support.microsoft.com/en-us/office/<function>-function-<id>`

**Reference Implementation (LibreOffice Calc):**
- Source: https://github.com/LibreOffice/core/tree/master/sc/source/core/tool
- Function implementations are in `interpr1.cxx` through `interpr8.cxx`
- Search for `ScInterpreter::Sc<FunctionName>` (e.g., `ScAggregate`)

Example to find ROUND implementation:
```bash
# Clone LibreOffice (or browse on GitHub)
git clone --depth 1 https://github.com/LibreOffice/core.git
grep -r "ScInterpreter::ScRound" core/sc/source/core/tool/
```

### 2. Implement the Function

**File:** `crates/duke-sheets-formula/src/functions/<category>.rs`

> **Note on Floating Point Precision:** Functions should return raw floating point results without rounding. Rounding/formatting is a display concern and is handled separately by the rendering layer. Tests should use approximate comparison for floating point values (e.g., `assert!((result - expected).abs() < 1e-10)`).

```rust
// In the appropriate category file (math.rs, text.rs, etc.)

pub fn fn_round(args: &[FormulaValue], _ctx: &EvaluationContext) -> FormulaResult<FormulaValue> {
    // 1. Validate argument count (handled by registry, but double-check)
    // 2. Extract and convert arguments
    // 3. Implement the logic (match Excel/LibreOffice behavior)
    // 4. Return FormulaValue or error
...
**File:** `crates/duke-sheets-formula/src/functions/mod.rs`

```rust
// In the appropriate register_*_functions() method
...
self.register(FunctionDef {
    ...
    volatile: false,  // true for NOW, TODAY, RAND, etc.
});
```

### 4. Add Tests

**File:** `crates/duke-sheets-formula/src/functions/<category>.rs` or `evaluator.rs`
...
fn test_round() {
    ...
    // Test edge cases (check LibreOffice wiki for these)
    assert_eq!(eval("=ROUND(-2.5, 0)").unwrap(), FormulaValue::Number(-3.0));
    assert_eq!(eval("=ROUND(1234.5, -2)").unwrap(), FormulaValue::Number(1200.0));
}
```

### 5. Update This Document

Mark the function as implemented in the table below.
