...
pub(crate) fn read_chart_ex<R: Read + Seek>(
    ...
    let mut buf = Vec::new();
    let parsed = parse_chart_ex_xml(&mut xml_reader, &mut buf)?;
    ...
    let mut cx = ChartEx {
        ...
        extensions: None,
        // external_data is parsed below from cx:chartData > cx:externalData
    };

    // These will be populated by the caller from the rels
    let _ = &mut cx;
...
fn parse_chart_ex_xml<R: Read>(
    xml_reader: &mut Reader<BufReader<R>>,
    buf: &mut Vec<u8>,
) -> XlsxResult<ParsedChartEx> {
                ...
                match tag {
                    b"externalData" if in_chart_data => {
                        ...
                        for attr in e.attributes().flatten() {
                            match attr.key.local_name().as_ref() {
                                b"id" => {
                                    rel_id = attr.unescape_value()
                                        .map(|s| s.to_string())
                                        .unwrap_or_default();
                                }
                                b"autoUpdate" => {
                                    auto_update = attr.unescape_value()
                                        .ok()
