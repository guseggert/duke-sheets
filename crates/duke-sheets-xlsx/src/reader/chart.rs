...
/// Read a chart XML part and return a `Chart` with the given anchor.
pub(crate) fn read_chart<R: Read + Seek>(
    ...
    let mut buf = Vec::new();
    let parsed = parse_chart_xml(&mut xml_reader, &mut buf)?;
...
fn parse_chart_xml<R: Read>(
    xml_reader: &mut Reader<BufReader<R>>,
    buf: &mut Vec<u8>,
) -> XlsxResult<ParsedChart> {
        ...
        match xml_reader.read_event_into(buf) {
            Ok(Event::Start(e)) => {
                ...
                match tag {
                    ...
                    b"numFmt" if (in_cat_ax || in_val_ax || in_ser_ax) && !in_ax_title && !in_dlbls => {
                    ...
                    b"spPr" if !in_sp_pr => {
                        ...
                        } else if in_ser && !in_dpt && !in_trendline && !in_err_bars && !in_marker && !in_dlbls {
            ...
            Ok(Event::End(e)) => {
                ...
                match tag {
                        ...
                        if in_chart_type_element =>
                    {
                        ...
                        let is_3d = chart_type_tag.as_deref().map_or(false, |t| t.contains("3D"));
                    ...
                    b"upDownBars" if in_up_down_bars => {
                        let up = up_bars_sp.take()
                            .map(|sp| ChartLines { shape_properties: Some(sp) })
                            .or(if had_up_bars { Some(ChartLines::default()) } else { None });
                        let down = down_bars_sp.take()
                            .map(|sp| ChartLines { shape_properties: Some(sp) })
                            .or(if had_down_bars { Some(ChartLines::default()) } else { None });
                    ...
                    b"spPr" if in_sp_pr => {
                        ...
                        if sp_pr_depth == 0 {
                            ...
                            let has_content = props.solid_fill.is_some()
                                || props.no_fill
                                || props.line.is_some();
                            if has_content {
                                match sp_pr_context {
                                    ...
                                    SpPrContext::DropLines => {
                                        group_drop_lines = Some(ChartLines { shape_properties: Some(props) });
                                    }
                                    SpPrContext::HiLowLines => {
                                        group_high_low_lines = Some(ChartLines { shape_properties: Some(props) });
                                    }
                                    SpPrContext::SerLines => {
                                        group_series_lines = Some(ChartLines { shape_properties: Some(props) });
                                    }
                                    SpPrContext::UpBars => {
                                    ...
                                    SpPrContext::LeaderLines => {
                                        dlbls.leader_lines = Some(ChartLines { shape_properties: Some(props) });
                                    }
                                }
                            }
                            in_sp_pr = false;
...
mod tests {
    ...
    fn parse_chart_xml_str(xml: &str) -> ParsedChart {
        let mut archive = zip_with_entry("xl/charts/chart1.xml", xml);
        let file = archive.by_name("xl/charts/chart1.xml").unwrap();
        let reader = std::io::BufReader::new(file);
        let mut xml_reader = Reader::from_reader(reader);
        xml_reader.config_mut().trim_text(true);
        let mut buf = Vec::new();
        parse_chart_xml(&mut xml_reader, &mut buf).unwrap()
    }
...
<c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"
  ...
  <c:chart>
    <c:title>
      <c:tx>
        <c:rich>
          <a:p><a:r><a:t>Revenue by Quarter</a:t></a:r></a:p>
        </c:rich>
      </c:tx>
    </c:title>
    <c:plotArea>
      ...
      <c:valAx>
        <c:axId val="1"/>
        <c:scaling><c:min val="0"/><c:max val="50000"/></c:scaling>
      </c:valAx>
    </c:plotArea>
    <c:legend>
      <c:legendPos val="r"/>
    ...
    #[test]
    fn test_xlsx_with_multiple_charts() {
        let mut bytes = Vec::new();
        {
            let cursor = Cursor::new(&mut bytes);
            let mut zip = zip::ZipWriter::new(cursor);
            ...
            zip.start_file("xl/worksheets/sheet1.xml", options).unwrap();
            zip.write_all(br#"<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetData/><drawing r:id="rId1"/></worksheet>"#).unwrap();
      ...
      <a:graphic>
        <a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/chart">
          <c:chart xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" r:id="rId2"/>
            ...
            zip.start_file("xl/drawings/_rels/drawing1.xml.rels", options)
                .unwrap();
            zip.write_all(br#"<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="../charts/chart1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="../charts/chart2.xml"/></Relationships>"#).unwrap();
...
<c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart">
  <c:chart><c:plotArea><c:lineChart>
    ...
    <c:ser><c:idx val="0"/><c:val><c:numRef><c:f>Sheet1!$A$1:$A$5</c:f></c:numRef></c:val></c:ser>
  </c:lineChart></c:plotArea></c:chart>
</c:chartSpace>"#,
            )
            .unwrap();

            zip.start_file("xl/charts/chart2.xml", options).unwrap();
            zip.write_all(
                br#"<?xml version="1.0"?>
<c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart">
  <c:chart><c:plotArea><c:pieChart>
    <c:ser><c:idx val="0"/><c:val><c:numRef><c:f>Sheet1!$B$1:$B$5</c:f></c:numRef></c:val></c:ser>
        ...
        assert_eq!(sheet.chart_count(), 2);

        let types: Vec<&ChartType> = sheet.charts().iter().map(|c| &c.chart_type).collect();
  ...
  <c:chart>
    <c:plotArea>
      ...
      <c:valAx>
        ...
        let parsed = parse_chart_xml_str(xml);
        let sp = parsed.value_axis.as_ref().unwrap().shape_properties.as_ref().unwrap();
...
<c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"
  ...
  <c:extLst>
    <c:ext uri="{chart-space-ext}"><spaceData/></c:ext>
        ...
        let ser_ext = parsed.series[0].raw_ext.as_ref().expect("series extLst missing");
        ...
        let ct_ext = parsed.raw_extensions.get("chartType").expect("chartType extLst missing");
        assert!(std::str::from_utf8(ct_ext).unwrap().contains("chart-type-ext"));
        ...
        let pa_ext = parsed.raw_extensions.get("plotArea").expect("plotArea extLst missing");
        assert!(std::str::from_utf8(pa_ext).unwrap().contains("plot-area-ext"));

        let ch_ext = parsed.raw_extensions.get("chart").expect("chart extLst missing");
        assert!(std::str::from_utf8(ch_ext).unwrap().contains("chart-ext"));

        let cs_ext = parsed.raw_extensions.get("chartSpace").expect("chartSpace extLst missing");
        assert!(std::str::from_utf8(cs_ext).unwrap().contains("chart-space-ext"));
    }
