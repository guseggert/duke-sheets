//! Decompile parsed formula tokens (RPN) into an infix formula string.
//!
...
use super::parsed_token::ParsedToken;
...
fn format_ref(row: u32, col: u16, row_rel: bool, col_rel: bool) -> String {
...
pub fn decompile(tokens: &[ParsedToken], ctx: &FormulaContext) -> String {
    ...
    for token in tokens {
        match token {
            ...
            ParsedToken::MissArg => stack.push(StackEntry::atom(String::new())),
            ParsedToken::Str(s) => {
            ...
            ParsedToken::Int(val) => stack.push(StackEntry::atom(val.to_string())),
            ParsedToken::Num(val) => stack.push(StackEntry::atom(format_number(*val))),
            ...
            ParsedToken::FuncVar { argc, func_idx } => {
                let raw_idx = *func_idx & 0x7FFF;
                let is_ce = (*func_idx & 0x8000) != 0 || raw_idx == 0xFF;
                if is_ce && *argc > 0 {
                    let arg_count = (*argc as usize) - 1;
                    let mut args: Vec<String> = Vec::with_capacity(arg_count);
                    for _ in 0..arg_count {
                        let entry = stack.pop().unwrap_or_else(|| StackEntry::atom("?".into()));
                        args.push(entry.text);
                    }
                    args.reverse();
                    let func_ref = stack.pop().unwrap_or_else(|| StackEntry::atom("?".into()));
                    let func_name = strip_xlfn_prefix(&func_ref.text);
            ...
            ParsedToken::Array { text } => stack.push(StackEntry::atom(text.clone())),
            ...
            ParsedToken::Table { row, col } => {
                let addr = duke_sheets_core::CellAddress::new(*row, *col).to_string();
...
fn resolve_sheet_prefix(ctx: &FormulaContext, extern_sheet_idx: u16) -> String {
    ...
    match supbook {
        SupBook::SelfRef { .. } => {
            // 0xFFFE = workbook-level (e.g. workbook-scoped name)
            // 0xFFFF = self-reference (formula's own sheet) - both need no prefix
...
///
/// If the context has a `base_cell`, the offset is added to it.
/// Without a base cell (shouldn't happen for valid shared formulas), the offset
/// is interpreted as an unsigned value (best effort).
fn resolve_refn_offset(ctx: &FormulaContext, row_off: i32, col_off: i16) -> (u32, u16) {
    match ctx.base_cell {
        Some((base_row, base_col)) => {
            let row = ((base_row as i64) + (row_off as i64)) as u32;
            let col = ((base_col as i32) + (col_off as i32)) as u16 & 0x3FFF;
            (row, col)
        }
        None => {
            // No base cell - treat offsets as unsigned (best effort for non-shared context)
            (row_off as u32, col_off as u16)
        }
    }
}

fn strip_xlfn_prefix(name: &str) -> &str {
    name.strip_prefix("_xlfn._xlws.")
...
#[cfg(test)]
mod tests {
    use super::super::{ExternSheetEntry, NameRecord, SupBook};
    use super::*;
