//! Built-in function table (485 entries).
//!
...
/// Function names indexed by function number.
...
static FTAB_ARGC: [u16; 485] = [
    ...
    2, 2, 7, 7, 3, 8, 5, 3, // 464-471
    3, 255, 255, 255, 2, 4, 5, 1, // 472-479
    2, 128, 129, 3, 129, // 480-484
];

static REVERSE_FTAB: OnceLock<HashMap<&'static str, u16>> = OnceLock::new();

/// Look up a function index by name (case-insensitive).
pub fn function_index(name: &str) -> Option<u16> {
    let map = REVERSE_FTAB.get_or_init(|| {
        let mut m = HashMap::new();
        for (i, &n) in FTAB.iter().enumerate() {
            if !n.is_empty() {
                m.insert(n, i as u16);
            }
        }
...
/// Look up the declared argument count for a function.
///
