...
fn pivot_needs_hidden_total_source(pivot: &PivotTable) -> bool {
    !pivot.layout.visual_totals || pivot.layout.subtotal_hidden_items
}
...
fn write_pivot_row_outline_levels(
...
fn clear_pivot_row_outline_levels(worksheet: &mut Worksheet, range: Option<CellRange>) {
...
fn snapshot_for_resolved_source(
    workbook: &Workbook,
...
struct CompiledPivotPlan {
    ...
    row_sort_measure_indexes: Vec<Option<usize>>,
    column_sort_measure_indexes: Vec<Option<usize>>,
...
impl CompiledFilter {
    ...
    fn targets_axis(&self, row_indexes: &[usize], column_indexes: &[usize]) -> bool {
...
struct PivotAggregation {
    ...
    matched_rows: usize,
    total_source: Option<Arc<PivotAggregation>>,
    subtotal_source: Option<Arc<PivotAggregation>>,
}
...
fn compile_axis_sort_measure_indexes(
    pivot_name: &str,
    fields: &[PivotField],
    measures: &[PivotMeasure],
...
fn values_on_rows(pivot: &PivotTable, plan: &CompiledPivotPlan) -> bool {
...
fn row_subtotal_rendered_count(
    plan: &CompiledPivotPlan,
    aggregation: &PivotAggregation,
    row_key: &[u32],
    position: usize,
...
fn render_values_on_rows_without_column_fields(
    pivot: &PivotTable,
    snapshot: &SourceSnapshot,
    plan: &CompiledPivotPlan,
    aggregation: &PivotAggregation,
...
fn values_on_rows_header(pivot: &PivotTable, plan: &CompiledPivotPlan) -> Vec<CellValue> {
    let mut header = row_field_header_cells(plan);
    header.insert(
...
fn values_on_rows_subtotal_label_cells(
    pivot: &PivotTable,
    snapshot: &SourceSnapshot,
    plan: &CompiledPivotPlan,
    prefix: &[u32],
...
fn column_slot_aggregate_override(slot: &ColumnRenderSlot) -> Option<PivotAggregate> {
    match slot {
...
fn total_source(aggregation: &PivotAggregation) -> &PivotAggregation {
    aggregation.total_source.as_deref().unwrap_or(aggregation)
}

fn subtotal_source(aggregation: &PivotAggregation) -> &PivotAggregation {
    aggregation
        .subtotal_source
        .as_deref()
...
fn grand_total_states(aggregation: &PivotAggregation) -> &Vec<AggregateState> {
...
fn row_subtotals_for_position(plan: &CompiledPivotPlan, position: usize) -> Vec<PivotSubtotal> {
    plan.row_fields
        .get(position)
...
fn column_subtotals_for_position(plan: &CompiledPivotPlan, position: usize) -> Vec<PivotSubtotal> {
    plan.column_fields
        .get(position)
        .map(enabled_subtotals_for_field)
        .unwrap_or_default()
...
fn enabled_subtotals_for_field(field: &PivotField) -> Vec<PivotSubtotal> {
...
fn row_group_end_positions(row_key: &[u32], next_row_key: Option<&Vec<u32>>) -> Vec<usize> {
    (0..row_key.len())
        .rev()
        .filter(|position| {
            next_row_key
...
fn is_row_subtotal_position(row_key: &[u32], position: usize) -> bool {
    position < row_key.len().saturating_sub(1)
}

fn append_blank_row_after_ended_row_field(
    cells: &mut Vec<Vec<CellValue>>,
    plan: &CompiledPivotPlan,
    position: usize,
    width: usize,
...
fn row_field_inserts_blank_row(plan: &CompiledPivotPlan, position: usize) -> bool {
    plan.row_fields
        .get(position)
...
fn values_caption(pivot: &PivotTable) -> &str {
...
fn finalize_measure_from_states(
    states: Option<&Vec<AggregateState>>,
    measures: &[PivotMeasure],
    row_total: Option<&Vec<AggregateState>>,
    column_total: Option<&Vec<AggregateState>>,
...
fn finalize_measure_from_state_slice(
    states: &[AggregateState],
    measures: &[PivotMeasure],
    row_total: &[AggregateState],
    column_total: &[AggregateState],
...
mod tests {
    use duke_sheets_core::{
        ...
        PivotSourceRange, PivotSubtotal, PivotTable, PivotValue, PivotValuesAxis, Table,
        TableColumn, Workbook,
    };
    use pretty_assertions::assert_eq;
    use ssfmt::{date_serial::date_to_serial, DateSystem};

    use super::{FormatPivotSource, PivotRefreshOptions, WorkbookPivotExt};
    ...
    fn refreshes_multiple_column_subtotal_functions() {
        ...
        let year =
            PivotField::new("Year").with_subtotals([PivotSubtotal::Average, PivotSubtotal::Max]);
        let pivot = PivotTable::builder("SalesPivot")
            .source_range(CellRange::parse("A1:D3").unwrap())
            .target_address("F1")
            .unwrap()
