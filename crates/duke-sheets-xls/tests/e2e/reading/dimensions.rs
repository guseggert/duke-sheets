...
#[test]
fn test_xls_custom_column_width() {
    skip_if_no_lo!();
    let path = temp_fixture_path();
    ...
    // LO's column width conversion is approximate -- allow generous tolerance
    assert!(
        width > 15.0,
...
#[test]
fn test_xls_multiple_row_heights() {
    skip_if_no_lo!();
    let path = temp_fixture_path();
