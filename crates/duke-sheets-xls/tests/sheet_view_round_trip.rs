...
fn lo_can_read_sheet_view_we_emit() {
    ...
    let mut wb = Workbook::new();
    let ws = wb.worksheet_mut(0).unwrap();
    ws.set_cell_value("A1", "view").expect("A1");
    ws.set_zoom_scale(Some(125));
    ws.add_selection(Selection {
        pane: None,
        active_cell: Some("B3".into()),
        sqref: Some("B3:C5".into()),
    });

    let bytes = XlsWriter::write_to_bytes(&wb).expect("serialize");
    std::fs::create_dir_all(SHARED_DIR).expect("shared dir");
    let pid = std::process::id();
    let path = format!("{SHARED_DIR}/duke_view_{pid}.xls");
    ...
    assert_eq!(outcome.expect("LO must open sheet-view workbook"), "view");
}
