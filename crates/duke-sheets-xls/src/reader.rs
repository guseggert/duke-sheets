...
use duke_sheets_core::{
    ...
    PivotValue, PivotValuesAxis, Style, Workbook,
};
...
impl XlsReader {
    ...
    pub fn read_with_password<R: Read + Seek>(
                ...
                0x00D2 if in_globals => {
                    if let Some(cache_num) = pending_pivot_cache_num {
                        pivot_cache_consolidation_page_refs
                            .entry(cache_num.saturating_sub(1))
                            .or_default()
                            .push(Self::parse_sxtbpg(&rec.data));
                    }
                }
        ...
        Self::apply_pivot_consolidation_page_items(
    ...
    fn parse_sheet_records(
        ...
        Self::parse_sheet_pivot_tables(records, ws, pivot_caches, &style_ctx.formats)?;
