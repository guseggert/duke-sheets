...
| Data connections (database) | R● W● | R✖ W✖ | R- W- | R- W- | `duke-sheets-xlsx::writer::tests::test_writer_round_trips_external_pivot_database_connection` | §18.13 CT_Connections/CT_Connection/CT_DbPr | Basic workbook `connections.xml` database connections with id, name, connection string, command text/type, and refresh flags round-trip; credentials, parameters, web/text/OLE DB extension details, external ODC files, and live refresh are not complete yet |
