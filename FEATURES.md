...
| Form control: button | R✔ W✔ | R✔ W✔ | R✔ W✔ | R✖ W✖ | `duke-sheets-xls::form_controls_round_trip::button_round_trips`, `excel_com_e2e::writing_xls::excel_can_read_form_controls_we_emit`, `xlsx_roundtrip::xlsx_form_controls_round_trip`, `excel_com_e2e::writing::excel_can_read_xlsx_form_controls_we_emit`, `duke-sheets-xlsb::writer::tests::form_controls_roundtrip`, `excel_com_e2e::writing_xlsb::excel_can_read_xlsb_form_controls_we_emit` | §18.3.1.26, MS-XLS §2.4.181 | **XLS**: caption via TXO; macro assignment (ftMacro) is not modeled — skipped on read, never written. Shape names persist in XLSX (`control/@name`) but not in XLS/XLSB, where Excel derives them from object ids. **XLSX**: kind/state/links live in `ctrlProps` parts + the worksheet `<controls>` block, captions in the legacy VML part; the a14 drawing-part twins Excel adds are omitted on write (Excel accepts this) and deduplicated on read. **XLSB**: controls are stored entirely in the legacy VML part referenced by BrtLegacyDrawing. |
