...
| Form control linked-cell synchronization | R✔ W✔ | R✔ W✔ | R✔ W✔ | R✖ W✖ | `form_control_link_sync::high_level_save_synchronizes_form_control_linked_cells`, `excel_com_e2e::writing_xls::excel_authored_xls_form_control_linked_cell_semantics`, `excel_com_e2e::writing::excel_can_read_xlsx_form_controls_we_emit`, `excel_com_e2e::writing_xls::excel_can_read_form_controls_we_emit`, `excel_com_e2e::writing_xlsb::excel_can_read_xlsb_form_controls_we_emit` | MS-XLS §2.5.141, §2.5.147, §2.5.153, §2.5.154 | High-level Excel-format save APIs serialize a synchronized snapshot, matching Excel: booleans / `#N/A` for checkboxes, one-based list/dropdown/radio indices, and numeric scrollbar/spinner values. Fresh unchecked/no-selection/multi-select links preserve a blank cell; existing linked values reset to `FALSE` or `0`. Existing formulas are overwritten in the serialized file; malformed, external-workbook, and unknown-sheet links are left unchanged. Low-level format writers remain pure; call `Workbook::sync_form_control_links` first when using them directly. |
