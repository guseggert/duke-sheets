...
| Intersection operator (space) | R✔ W✔ | R✔ W✔ | R✔ W✔ | R✖ W✖ | `parser::tests::test_parse_intersection`, `parser::tests::test_parse_intersection_in_function`, `excel_com_e2e::writing::excel_can_evaluate_intersection_we_emit`, `excel_com_e2e::writing_xls::excel_can_evaluate_intersection_we_emit`, `excel_com_e2e::writing_xlsb::excel_can_evaluate_intersection_we_emit` | §18.17.2.2 | Tokeniser emits Token::Space when whitespace separates two value-producing tokens; parser folds it into BinaryOp(Intersect). XLSB compiler emits PtgIsect with R-class operand areas. Our own evaluator returns an error for the operator (Excel does the math). |
| Range union operator (comma) | R✔ W✔ | R✔ W✔ | R✔ W✔ | R✖ W✖ | `parser::tests::test_parse_union_in_parens`, `excel_com_e2e::writing::excel_can_evaluate_union_we_emit`, `excel_com_e2e::writing_xls::excel_can_evaluate_union_we_emit`, `excel_com_e2e::writing_xlsb::excel_can_evaluate_union_we_emit` | §18.17.2.2 | Parser distinguishes function-call commas from union commas: bare parens (`(A,B)`) fold the comma into BinaryOp(Union); function-call parens (`SUM(A,B)`) keep the existing arg-separator behaviour. XLSB compiler emits PTG_LIST. |
...
| Feature | XLSX | XLSB | XLS | ODS | Test | Spec |
|---------|------|------|-----|-----|------|------|
| Image parsing (PNG/JPEG) | R✔ W✔ | R✖ W✖ | R● W● | R✖ W✖ | `xlsx_roundtrip::xlsx_png_image_round_trips`, `duke-sheets-xls::pictures_round_trip::single_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::single_jpeg_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::single_bmp_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::single_emf_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::single_wmf_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::single_tiff_picture_round_trips`, `duke-sheets-xls::pictures_round_trip::gif_picture_falls_back_to_png_format`, `duke-sheets-xls::pictures_round_trip::picture_anchor_within_cell_offsets_round_trip`, `duke-sheets-xls::pictures_round_trip::rotation_and_flip_flags_round_trip`, `excel_com_e2e::writing::excel_can_read_xlsx_png_image_we_emit`, `excel_com_e2e::writing_xls::excel_can_read_xls_png_image_we_emit`, `excel_com_e2e::writing_xls::excel_can_read_xls_jpeg_image_we_emit`, `excel_com_e2e::writing_xls::excel_can_read_xls_bmp_image_we_emit`, `excel_com_e2e::writing_xls::excel_can_read_xls_picture_rotation_and_flip_we_emit` | §20.4 (DrawingML), MS-ODRAW | XLS supports PNG / JPEG / BMP / EMF / WMF / TIFF end-to-end. PNG, JPEG, TIFF use the corresponding raster Blip variants. BMP routes through OfficeArtBlipDIB (writer strips the 14-byte BITMAPFILEHEADER, reader synthesises one back). EMF/WMF use OfficeArtBlipEMF/WMF with the 34-byte metafileHeader; Excel re-renders metafiles as PNG on SaveAs so byte equality through Excel is not preserved, but in-process round-trip is exact. Within-cell EMU offsets round-trip via the 1024ths/256ths anchor quantisation; width/height in EMU are synthesised on read from the anchor cell range × default cell sizes. Rotation (60,000ths of a degree, FOPT 0x0004) and flip H/V (FSP grfPersistence bits) round-trip exactly. GIF input has no native Office binary blip variant (MS-ODRAW's msoblip* enum skips it); the writer emits GIF bytes through the PNG blip path so the bytes survive round-trip but the format tag flips to PNG. |
...
| Image positioning (one-cell anchor) | R✖ W✖ | R✖ W✖ | R● W● | R✖ W✖ | `duke-sheets-xls::pictures_round_trip::onecell_anchor_round_trips_with_visual_area_preserved`, `excel_com_e2e::writing_xls::excel_can_read_xls_onecell_image_we_emit` | §20.4.2.1 | XLS ClientAnchor is one byte layout shared across all OOXML anchor variants; OneCell input collapses to TwoCell with editAs=OneCell on read. Visual area (cells covered) is preserved via flag=2 (move only) and width/height encoded into the to-cell offset. |
| Image positioning (absolute anchor) | R✖ W✖ | R✖ W✖ | R● W● | R✖ W✖ | `duke-sheets-xls::pictures_round_trip::absolute_anchor_round_trips_with_visual_area_preserved`, `excel_com_e2e::writing_xls::excel_can_read_xls_absolute_image_we_emit` | §20.4.2.3 | Absolute input collapses to TwoCell with editAs=Absolute on read; the x_emu/y_emu position lands the picture at the equivalent cell at default cell sizes. flag=3 (no move, no resize) signals the absolute semantics to readers. |
| Image editAs (move/size with cells) | R✖ W✖ | R✖ W✖ | R✔ W✔ | R✖ W✖ | `duke-sheets-xls::pictures_round_trip::twocell_anchor_edit_as_round_trips`, `duke-sheets-xls::pictures_round_trip::onecell_anchor_round_trips_with_visual_area_preserved`, `duke-sheets-xls::pictures_round_trip::absolute_anchor_round_trips_with_visual_area_preserved` | §20.4.2.8 | editAs preserved via the ClientAnchor flag bit: TwoCell=0, OneCell=2, Absolute=3. |
