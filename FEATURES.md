...
| Dynamic filter (above/below avg, etc.) | R✔ W✔ | R✔ W● | R✔ W✖ | R✖ W✖ | `xlsx_roundtrip::test_auto_filter_dynamic`, `duke-sheets-xlsb::writer::tests::dynamic_filter_in_process_roundtrip` | §18.3.2.5 | XLSB writer/reader pair round-trips internally but the BrtDynamicFilter record ID is not authoritatively published; Excel rejects files emitted at our suspected ID (0x00A8) with the spec-derived 21-byte payload. Need a real-world Excel-emitted reference to pin the right ID. |
