...
| Intersection operator (space) | R✔ W✔ | R✔ W✔ | R● W● | R✖ W✖ | `parser::tests::test_parse_intersection`, `parser::tests::test_parse_intersection_in_function`, `xls_formula_round_trip::intersection_formula_text_is_lost_through_xls_roundtrip`, `excel_com_e2e::writing::excel_can_evaluate_intersection_we_emit`, `excel_com_e2e::writing_xls::excel_can_evaluate_intersection_we_emit`, `excel_com_e2e::writing_xlsb::excel_can_evaluate_intersection_we_emit` | §18.17.2.2 | Tokeniser emits Token::Space when whitespace separates two value-producing tokens; parser folds it into BinaryOp(Intersect). XLSB compiler emits PtgIsect with R-class operand areas; XLSX emits the operator in formula text. Our own evaluator returns an error for the operator (Excel does the math). **XLS limitation**: the writer→reader round-trip drops the formula text entirely (the cached value survives but the cell loses its formula). The XLS compiler does not emit a working PtgIsect; pinned by intersection_formula_text_is_lost_through_xls_roundtrip. The Excel parity test for XLS only verifies the cached value, not the formula structure. |
| Range union operator (comma) | R✔ W✔ | R✔ W✔ | R● W● | R✖ W✖ | `parser::tests::test_parse_union_in_parens`, `xls_formula_round_trip::union_formula_text_is_lost_through_xls_roundtrip`, `excel_com_e2e::writing::excel_can_evaluate_union_we_emit`, `excel_com_e2e::writing_xls::excel_can_evaluate_union_we_emit`, `excel_com_e2e::writing_xlsb::excel_can_evaluate_union_we_emit` | §18.17.2.2 | Parser distinguishes function-call commas from union commas: bare parens (`(A,B)`) fold the comma into BinaryOp(Union); function-call parens (`SUM(A,B)`) keep the existing arg-separator behaviour. XLSB compiler emits PTG_LIST. **XLS limitation**: same as the intersection row above — writer→reader round-trip drops the formula text. The XLS compiler does not emit a working PTG_LIST; pinned by union_formula_text_is_lost_through_xls_roundtrip. |
