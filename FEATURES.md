...
| Form control: option (radio) button | R✖ W✖ | R✖ W✖ | R● W● | R✖ W✖ | `duke-sheets-xls::form_controls_round_trip::option_buttons_round_trip_as_group`, `excel_com_e2e::writing_xls::excel_can_read_form_controls_we_emit` | MS-XLS §2.5.153 | **XLS**: caption, state, cell link. The writer chains all option buttons on a sheet into a single circular group in insertion order (fFirstBtn on the first); grouping by enclosing group box is not modeled. |
