    ...
    try
    {
        switch (req.Cmd)
        {
            ...
            case "Get":
            {
                ...
                var (isHandle, h, val) = store.GetProperty(target, property);

                object? data = isHandle ? new HandleData(h) : new ValueData(val);
            ...
            case "Invoke":
            {
                ...
                var target = store.Navigate(handle, chain);
                var (isHandle, h, val) = store.InvokeMethod(target, method, args);

                object? data = isHandle ? new HandleData(h) : val != null ? new ValueData(val) : null;
                return (Response.Ok(id, data), false);
            }
